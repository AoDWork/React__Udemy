...
{/*    ====    08. Пагинация данных (дозагрузка персонажей)     ====
    ...
    При клике на кнопку должен уходить запрос на загрузку следующих 9 персонажей, тоесть в сервисес offset должен меняться. Новые
        ...
        Теперь будем вызывать onRequest по клику на кнопку и чтобы при этом передавалось число на 9 больше чем предыдущее.
        Есть вариант увеличивать басе офсет в сервисе, но так получиться загрязнение нашего компонента сервис лишней логикой,
        потому что это локальное состояние для CharList, значит поместим в его state новое свойство offset. Каждый раз когда
        завершается запрос на сервер будем увеличивать это число на 9. Сделаем это в onCharListLoaded. Вытащим свойство из state
        и будем добавлять 9 к старому его значению, таким образом после первоначальной загрузки списка offset уже будет 219.
        ...
        Добавим к кнопке вызов метода, также будем устанавливать аттрибут disabled в зависимости от newItemsLoading. Возьмем из
        state нужные свойства newItemsLoading и offset

            render() {
                const {charList, loading, error, offset, newItemsLoading} = this.state;
        ...
        Такой метод используется во всех случаях когда необходима дозагрузка контента. Есть еще способ дозагружать не через клик по 
        кнопке, а при долистывании странцы до конца. Ранее в JS курсе был рассмотрен скрипт который  определяет конец страницы. 

        
        Пока что кнопка хоть и становится disabled но внешне это никак не отображается, нужно добавить стили. Откроем стили для 
        кнопки в папке style - button. Скажем что когда у основной кнопки будет аттрибут disabled тогда применить фильтр серого на
        80%.
    ...
    Последнее что нужно сделать - предусмотреть действия на случай когда подгружаемые персонажи закончаться. Чтобы посмотреть какой
        последний персонаж, перейдем на сайт и сделаем тестовый запрос, таким образом узнаем общее количество персонажей. В видео
        total 1559, когда я делал запрос их уже 1563. Теперь попробуем что будет если мы сделаем запрос на несуществующих персонажей
        для этого изменим offset в видео 1560, для себя поставлю 1570. Первые загружаются нормально, а при клике больше не происходит
        подгрузка, потому что приходит пустой массив и юзер не видит никакого сообщения или ошибки. Чтобы не было ошибки в этому нам
        помогает бекенд, и теперь когда мы получим пустой массив можно скрыть кнопку чтобы юзер больше не нажимал ее. Если удалять
        кнопку то нужно перерисовывать часть компонента, головняк. Значит скроем, Иван попробовал через hidden и через style. вроде
        как лучше через инлайн стили style.

        Узнать что закончились персонажи можно или в сервисе математическим путем, но тогда мы не сможем их отобразить или проверкой на
        длинну массива в CharList при получении нового массива, добавить их к массиву и показать, а потом уже на следующий клик скрывать
        кнопку, для этого сделаем дополнительное свойство в state - charEnded: false. 

        Будем проверять меньше ли новый массив 9ти, если меньше тогда присвоим переменной true и при записи в state для charEnded
        занесеться true, а для кнопки запишем инлайн стиль display значение которого будет вычисляться в зависимости от значения
        charEnded, и когда оно будет true подставиться none и кнопка скроется, а при false display будет block.
            ...
            если будет пустой массив то кнопка просто исчезнет, если будет больше меньше 9ти то догрузяться персонажи и она пропадет
            но если точно последние 9 прийдут то кнопка останется и пропадет только при следующем клике, также без всякого сообщения
            можно доработать это по желанию.


    
    ТЕОРЕТИЧЕСКИЕ МОМЕНТЫ

    //! при реализации дозагрузки по скролу нужно навешивать обработчик события на window в componentDidMount и если это делается
    //! в реакте то нужно отменять обработчик в componentWillUnmount - window.removeEventListener чтобы не было утечки памяти.

    //! как сохранить данные которые были загружены, например если перешли на другую страницу и потом вернулись на эту и хотим 
    //! увидеть загруженную структуру. Один из вариантов (изучим в следующих уроках) - когда состояние приложения будет храниться
    //! выше чем компонент, в общем хранилище, и мы в компонент будем получать данные из него и есть вариант с использованием
    //! localStorage, например чтобы при заходе на странцу подгружались персонажи начиная со стандартного offset до сохраненного
    //! offset, для этого нужно эту цифру вынести в отдельную переменную(получать из локал сторадж) и передавать как отдельный  
    //! аргумент вместе с offsetom.
...
{/*    ====    09. Проверка типов с помощью PropTypes     ====
    ...
    В этом уроке будем использовать PropTypes, разрабатывать поще когда используешь статическую типизацию и точно знаешь какой тип
        данных должен прийти, для этого исользуют TypeScript но это отдельная технология которую нужно отдельно учить. А пока 
        можно использовать PropTypes для отслеживания типов данных внутри пропсов. Дока или на сайте реакта или на сайте npm 
        можно проверять на все типы + на структуру(shape), можно написать свой валидатор, установить isRequired - обязательный.

        //! На собесах спрашивают - можно ли установить props по умолчанию, если не было передано props. Да есть такая опция - 
        //! defaultProps 

        До реакт 15.5 эта возможность была включена в реакт, а теперь нужно устанавливать отдельную библиотеку prop-types, установим
        библиотеку в проект    
        ...
        теперь можем прописывать тип пропсов, и если переданные пропс не подходят по типам то будет показываться уведомление в 
        терминале, это работает только в режиме разработки, в продашене этих уведомлений не будет. Тестировать будем на CharInfo



    В CharInfo и портируем специальную сущность PropTypes

            import PropTypes from 'prop-types'

        теперь можно брать компонент CharInfo и проверять props которые в него приходят.

            CharInfo.propTypes = {
                charId: PropTypes.number
            }

        такая структура описывает что тип пропса charId будет number, если написать string то в консоли будет предупреждение.
